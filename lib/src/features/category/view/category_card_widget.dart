...
import 'package:midnight_suspense/src/constants/custom_painters/square_box_bottom_glow_gradient.dart';
...
class CategoryCardWidget extends StatelessWidget {
  ...
  CategoryCardWidget({
    super.key,
    required this.model,
  }) : imageProvider = CachedNetworkImageProvider(model.artUrl ?? "");

  Future<Color> getPrimaryColor() async {
    logger.i("model artUrl: ${model.artUrl}");
    if (model.artUrl == null) return Colors.red;
    final colorPalette = await getPalette(imageProvider);
    colorPalette.selectedSwatches.forEach((key, value) {
      logger.i("key: $key colors $value");
    });
    logger.i("colors ${colorPalette.targets}");

    return colorPalette.lightVibrantColor?.color ?? Colors.red;
  }
    ...
    return Material(
      ...
      child: FutureBuilder(
          ...
          builder: (context, snap) {
            var primaryColor = Colors.red.shade500;
            if (snap.hasData) {
              logger.i("snap data ${snap.data}");
              primaryColor = snap.data as Color;
              logger.i("primarycolor ${primaryColor.value}");
            }
              ...
              children: [
                ...
                Positioned.fill(
                  child: CustomPaint(
                    ...
                    size: Size(331, 203),
                    painter: SquareBoxBottomGlowGradientCustomPainter(color: primaryColor),
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            "\n\n" + model.title,
                            style: GoogleFonts.creepster(color: primaryColor),
                          ),
                          Text(
                            model.language.name,
                            style: GoogleFonts.ptSerif(color: primaryColor),
                          ),
                        ],
                      ),
                    ),
...
//Add this CustomPaint widget to the Widget Tree
// CustomPaint(
//     size: Size(WIDTH, (WIDTH*0.6382978723404256).toDouble()), //You can Replace [WIDTH] with your desired width for Custom Paint and height will be calculated automatically
...
class CheckPalleteGenerator extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // final image = Assets.images.hauntedHouseFrame2.provider();
    return MaterialApp(
      title: 'Image Colors',
      theme: ThemeData(
      ...
      home: ImageColors(
        title: 'Image Colors',
        image: CachedNetworkImageProvider("https://i3.ytimg.com/vi/v5thOM-_c7M/maxresdefault.jpg"),
...
class _ImageColorsState extends State<ImageColors> {
  ...
  Future<void> _updatePaletteGenerator(Rect? newRegion) async {
    paletteGenerator = await PaletteGenerator.fromImageProvider(widget.image,
        ...
        // ],
        targets: [
          PaletteTarget(minimumLightness: 1.0, minimumSaturation: 1.0),
          PaletteTarget(maximumLightness: 1.0, maximumSaturation: 1.0),
        ]);
    setState(() {});
  }
